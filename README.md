...
> 当前脚本版本：`v0.0.6`。项目需要 root 权限，会修改服务、证书、Nginx、防火墙和部分内核网络参数。请先了解变更范围，并仅用于合法、合规的网络用途。
...
备份包包含代理凭据、用户信息和证书私钥，默认权限为 `600`。请及时下载到可信设备，并进行加密保存。恢复前建议在可信设备上记录 SHA-256，并在目标 VPS 上显式校验：

```bash
VLESS_RESTORE_SHA256="这里填写备份的 SHA-256" vless --restore /root/singingbox-backup.tar.gz
```

恢复过程会拒绝路径穿越、符号链接、设备文件和非预期目录，并先暂存后切换配置目录。已有配置与 ACME 状态会保留为带时间戳的 `before-restore` 目录，确认新服务正常后再自行清理。
...
可在主菜单选择“检查脚本更新”，也可以重新下载最新版 `songbox.sh`。脚本会保留旧脚本备份，并尝试同步系统快捷命令；自动同步不会用较旧或同版本但内容不同的文件覆盖现有系统脚本。脚本更新默认只使用 GitHub Raw，第三方下载镜像不会自动启用。
